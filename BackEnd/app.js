...
app.post('/insertaSupervisor', async(req, res) => {
  try {
    const {nombre, apellidoP, apellidoM, fechaNacm, sexo, correo, nombreUsario, contra} = req.body;
    ...
    request.input('NombreUsario', mssql.VarChar(40), nombreUsario);
...
app.post('/loginSupervisor', async(req, res) => {
  try {
    ...
    const request = pool.request();
    request.input('NombreUsario', mssql.VarChar(40), nombreUsuario);
    request.input('Contraseña', mssql.VarChar(96), contra);
...
app.post('/insertaComensalCond', async(req, res) => {
  try {
    const {nombre, apellidoP, apellidoM, curp, fechaNacm, sexo, token, nombreCond} = req.body;
    ...
    // ALGORITMO PARA GENERAR EL TOKEN
    ...
    request2.input('NombreCond', mssql.VarChar(30), nombreCond);
    // FOR DONDE ITERE SOBRE req.body A PARTIR DE CIERTO ELEMENTO PARA LLENAR LAS CONDICIONES
    const result2 = await request2.execute('PROC_ICondCom');

    // REGRESAR EL TOKEN return res.status(201).send('OK', {token});
    return res.status(201).send('OK');
...
app.put('/actualizaEmpleado', async (req, res) => {
  try {
    const {idEmp, nombre, apellidoP, apellidoM, fechaNacm, sexo, correo, nombreUsario} = req.body;
...
app.delete('/eliminaEmpleado', async (req, res) => {
  try {
    ...
    const request = pool.request();
    request.input('NombreUsario', mssql.VarChar(40), nombre);
...
/*---------------- APP VOL/ENC ---------------- */
// Tabla sobre datos generales del comedor
app.get('/datosCom/:nombreUsuario', async (req, res) => {
  try {
    ...
    const request = pool.request();
    request.input('NombreUsario', mssql.VarChar(40), nombreUsuario)
    const result = await request.execute('PROC_CDatosComedor');
    const table = result.recordset;
