...
@NombreComedor AS VARCHAR(30), @Tipo AS VARCHAR(30), @Descripcion AS VARCHAR(60),
@Fecha AS DATE, @Hora AS TIME
AS
BEGIN	
	DECLARE @ComedorBusq AS INT;
	...
	VALUES (@ComedorBusq, @Tipo, @Descripcion, @Fecha, @Hora)
END;
GO
...
@NombreComedor AS VARCHAR(30), @Token AS INT, @TipoRacion AS VARCHAR(20),
@Raciones AS INT, @Fecha AS DATE, @Hora AS TIME
AS
BEGIN
	DECLARE @ComedorBusq AS INT;
...
AS
BEGIN
	SELECT IDComedor, Empleado.Correo, EstadoComedor.Nombre, Comedor.Nombre, Capacidad, Direccion, Telefono
	FROM Comedor
...
AS
BEGIN	
	SELECT Nombre, Tipo, Descripcion, Fecha, Hora 
	FROM Incidencias
...
--Catálogo "Sexo"
CREATE OR ALTER PROCEDURE PROC_CSexos
AS
BEGIN
	SELECT Descripcion
	FROM Sexo
...
--Catálogo "Tipo de Ración"
CREATE OR ALTER PROCEDURE PROC_CTipoRacion
AS
BEGIN
	SELECT Tipo
	FROM TipoRacion
...
--SP
SELECT Nombre, Pregunta, Comentarios, Calificacion, Fecha
FROM Encuesta
JOIN Pregunta ON Encuesta.IDPregunta = Pregunta.IDPregunta
JOIN Comedor ON Encuesta.IDComedor = Comedor.IDComedor
WHERE (Encuesta.IDComedor = 1)
*/
