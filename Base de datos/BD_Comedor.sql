...
BEGIN
	DECLARE @ContraseñaGuardada AS VARCHAR(96);
	SELECT @ContraseñaGuardada = (SELECT Contraseña FROM Empleado WHERE 
	(NombreUsuario LIKE @NombreUsuario) AND (IDRol = 2));
...
BEGIN
    ...
    INSERT INTO #Resultados (Metrica, Valor)
    SELECT 'Raciones servidas pagadas', COUNT(Raciones)
    FROM Asistencia
    WHERE (IDComedor = @ComedorBusq) AND (IDTipoRacion = 1) AND (Fecha = @Fecha);

    -- Raciones donadas
    INSERT INTO #Resultados (Metrica, Valor)
    SELECT 'Raciones donadas', COUNT(Raciones)
    FROM Asistencia
    WHERE (IDComedor = @ComedorBusq) AND (IDTipoRacion = 2) AND (Fecha = @Fecha);

    -- Número total de comensales
    INSERT INTO #Resultados (Metrica, Valor)
    SELECT 'Número total de comensales', COUNT(Raciones)
    FROM Asistencia
    WHERE (IDComedor = @ComedorBusq) AND (Fecha = @Fecha);

    -- Dinero Recaudado
    INSERT INTO #Resultados (Metrica, Valor)
    SELECT 'Dinero Recaudado', SUM(Raciones) * 13
    FROM Asistencia
    WHERE (IDComedor = @ComedorBusq) AND (IDTipoRacion = 1) AND (Fecha = @Fecha);
...
CREATE OR ALTER PROCEDURE PROC_CDashBoardLeader
@Fecha AS DATE
AS
BEGIN
	SELECT Nombre,
...
EXECUTE PROC_LoginEmpE 'FernandezLaura', 'ComedorQI', @Exito1 OUTPUT;
SELECT @Exito1;
GO
...
--<Sección de "Dashboard">
EXECUTE PROC_CDashBoard 'Santiago Tlatelolco', '2023-09-17';
GO
